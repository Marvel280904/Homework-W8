...
namespace Homework_W8
{
    public partial class Form1 : Form
    {
        ...
        {
            simpen = box2.SelectedValue.ToString();
            a3 = new DataTable();
            namadipilih = box2.GetItemText(box2.SelectedItem).ToString(); timpilih = box1.GetItemText(box1.SelectedItem).ToString();
            sql = "SELECT player_name as `Player Name`, team_name as `Team Name`, playing_pos as `Position`, nation as `Nationality`, team_number as `Squad Number` FROM player p, team t, nationality n\nwhere p.player_name = \"" + namadipilih + "\" and t.team_name = \"" + timpilih + "\" and p.nationality_id = n.nationality_id; ";
            SqlCommand = new MySqlCommand(sql, SqlConnection);
            SqlDataAdapter = new MySqlDataAdapter(SqlCommand);
